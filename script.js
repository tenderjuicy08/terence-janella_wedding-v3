document.addEventListener("DOMContentLoaded", function () {
  ...
  const countdownComplete =
    document.getElementById("countdownComplete");
  ...
  if (
    ...
    function updateAlbumDisplay() {
      ...
      albumCounter.textContent =
        `${currentAlbumIndex + 1} / ${albumCards.length}`;
  ...
  /* =========================================================
     ...
     BEHAVIOR:
     - Duplicate checking is based on FULL NAME.
     - Same email may be used for different guests.
     - NO localStorage duplicate blocking.
     - If a guest row is deleted from Google Sheets,
       that guest may RSVP again.
     - Google Apps Script performs the final duplicate check.
     ========================================================= */
  ...
  const rsvpSubmissionFrame =
    document.getElementById(
      "rsvpSubmissionFrame"
    );


  let rsvpRequestInProgress =
    false;
  ...
  /* =========================================================
     ...
     an old cached duplicate result.
     ========================================================= */

  function tryRemoteDuplicateCheck(
    fullName,
    timeoutMs = 5000
  ) {
    ...
    return new Promise(
      function (resolve) {
        ...
        const callbackName =
          `__weddingRsvpCallback_${Date.now()}_${
            Math.random()
              .toString(36)
              .slice(2)
          }`;
  ...
  function submitRsvpToHiddenFrame() {
    ...
    rsvpForm.target =
      rsvpSubmissionFrame.name;


    rsvpForm.acceptCharset =
      "UTF-8";
      ...
      async function (
        ...
        try {
          ...
          /* ===================================================
             ...
             NO localStorage is checked here.

             Therefore:

             If the name exists in the Sheet:
             BLOCK RSVP.

             If the name was deleted from the Sheet:
             ALLOW RSVP AGAIN.
             =================================================== */

          const remoteCheck =
            await tryRemoteDuplicateCheck(
              submittedRsvpData
                .fullName
            );
          ...
          /*
           * Apps Script performs the final
           * duplicate check and saves the row.
           */
          await delay(
            2200
          );
          ...
          if (
            ...
            /*
             * Some Messenger/in-app browsers
             * can block the duplicate lookup.
             *
             * The POST still goes to Apps Script.
             */

            showRsvpStatus(
              "success",

              `Thank you, ${guestName}! Your RSVP has been sent. The Google Sheet will determine whether this is a new or duplicate RSVP.`
            );
