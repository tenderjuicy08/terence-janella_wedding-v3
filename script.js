document.addEventListener("DOMContentLoaded", function () {
  ...
  /* RSVP FORM
     ---------------------------------------------------------
     IMPORTANT: Keep your actual Google Apps Script Web App
     deployment URL below. It must end in /exec.

     This version avoids relying on cross-origin postMessage.
     It checks duplicates through JSONP, submits through the
     hidden iframe, then verifies that the email now exists in
     the Google Sheet before showing success.
     --------------------------------------------------------- */
  ...
  function checkRsvpEmail(email, timeoutMs = 12000) {
    return new Promise(function (resolve, reject) {
          ...
          reject(
            new Error(
              payload && payload.message
                ? payload.message
                : "The RSVP server could not verify this email."
            )
          );
          return;
        }

        resolve(Boolean(payload.exists));
      };
      ...
      script.onerror = function () {
        cleanup();
        reject(new Error("The RSVP server could not be reached."));
      };

      const timeoutId = window.setTimeout(function () {
        cleanup();
        reject(new Error("The RSVP server did not respond in time."));
      }, timeoutMs);
  ...
  /* Start the POST request, but DO NOT wait for the hidden iframe's
     load event. Some mobile/in-app browsers do not report that event
     even though Google Apps Script successfully saves the RSVP. */
  ...
  /* Poll the duplicate-check endpoint until the just-submitted email
     appears in Google Sheets. This becomes the actual confirmation
     that the RSVP was saved. */
  async function waitForRsvpConfirmation(
    ...
    while (Date.now() - startedAt < maxWaitMs) {
      try {
        const exists = await checkRsvpEmail(email, 8000);

        if (exists) {
          return true;
        }
      } catch (error) {
        /* A temporary JSONP/network failure should not immediately
           turn a successful RSVP into an error. Keep checking until
    ...
    if (lastError) {
      console.warn("Last RSVP confirmation check error:", lastError);
    }
  ...
  if (rsvpForm && rsvpSubmissionFrame) {
    rsvpForm.addEventListener("submit", async function (event) {
      ...
      showRsvpStatus("loading", "Checking whether this email already has an RSVP...");

      try {
        const alreadyExists = await checkRsvpEmail(submittedRsvpData.email);
        ...
        /* Start the submission and immediately switch to confirmation.
           We intentionally do not wait for the iframe load event. */
        submitRsvpToHiddenFrame();

        showRsvpStatus("loading", "Confirming your RSVP...");

        const nowExists = await waitForRsvpConfirmation(
          submittedRsvpData.email
        );

        if (!nowExists) {
          throw new Error(
            "Your RSVP was sent, but the website could not confirm it yet. Please check the RSVP list before submitting again."
          );
        }
