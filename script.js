document.addEventListener("DOMContentLoaded", function () {
  ...
  /* RSVP FORM
     ...
     - Server-side duplicate protection prevents a second row.
     - A local device cache also catches repeat submissions made
       from the same browser immediately.
     --------------------------------------------------------- */
  ...
  function localRsvpKey(email) {
    return `wedding-rsvp:${normalizeRsvpEmail(email)}`;
  }

  function wasSubmittedOnThisDevice(email) {
    try {
      return window.localStorage.getItem(localRsvpKey(email)) === "submitted";
    } catch (error) {
      return false;
    }
  }

  function rememberSubmissionOnThisDevice(email) {
    try {
      window.localStorage.setItem(localRsvpKey(email), "submitted");
    } catch (error) {
  ...
  function tryRemoteDuplicateCheck(email, timeoutMs = 5000) {
    return new Promise(function (resolve) {
      ...
      script.src = buildRsvpUrl({
        action: "check",
        email: normalizeRsvpEmail(email),
  ...
  if (rsvpForm && rsvpSubmissionFrame) {
    rsvpForm.addEventListener("submit", async function (event) {
      ...
      /* Instant duplicate protection for repeats from the same device. */
      if (wasSubmittedOnThisDevice(submittedRsvpData.email)) {
        showRsvpStatus(
          "duplicate",
          "An RSVP using this email address was already submitted from this device. No duplicate entry was added."
        );
        return;
      }
      ...
      try {
        ...
        const remoteCheck = await tryRemoteDuplicateCheck(submittedRsvpData.email);

        if (remoteCheck.reachable && remoteCheck.exists) {
          rememberSubmissionOnThisDevice(submittedRsvpData.email);
          showRsvpStatus(
            "duplicate",
        ...
        if (remoteCheck.reachable) {
          ...
          showRsvpStatus(
            "success",
            `Thank you, ${guestName}! Your RSVP has been sent. Duplicate protection is active, so if this email was already registered, no additional entry will be created.`
          );
        }
